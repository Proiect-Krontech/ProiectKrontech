...
void SensorManager::begin() {
    LOG_DEBUG(TAG, "Initializare 4 celule de sarcina...");
    _fl->begin();
    _fr->begin();
    _bl->begin();
    _br->begin();
    LOG_DEBUG(TAG, "begin() complet.");
}

void SensorManager::wait_until_ready() {
    LOG_INFO(TAG, "Astept module HX711...");
    uint32_t waited = 0;
    ...
    LOG_INFO(TAG, "Module gata. (%lu ms)", waited);
}

void SensorManager::tare(uint8_t samples) {
    LOG_DEBUG(TAG, "Tara cu %d samples...", samples);
    ...
    LOG_DEBUG(TAG, "Tara completa -> FL:%ld  FR:%ld  BL:%ld  BR:%ld",
             z_fl, z_fr, z_bl, z_br);
}
...
void SensorManager::calibrate(uint8_t samples,
    ...
    LOG_DEBUG(TAG, "Calibrare cu %d samples, ref=%.1f kg...", samples, reference_weight_kg);
    ...
    LOG_DEBUG(TAG, "Raw net -> FL:%ld  FR:%ld  BL:%ld  BR:%ld  total:%ld",
    ...
    LOG_DEBUG(TAG, "Factor: %.4f | FL:%.1f%%  FR:%.1f%%  BL:%.1f%%  BR:%.1f%%",
         out_factor, out_ref.fl_pct, out_ref.fr_pct, out_ref.bl_pct, out_ref.br_pct);
}
